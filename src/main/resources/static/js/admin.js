...
document.addEventListener("DOMContentLoaded", ()=> {
    document.querySelectorAll(".btn-edit").forEach( btn => {
        btn.addEventListener("click", (e)=>{
            ...
            const row = btn.closest("tr"); // 현재 클릭된 수정버친의 tr 엘리먼트를 찾음
            const id = row.getAttribute("data-id"); // 해당 행(row)으로부터 id값을 설정함
            ...
            // 편집 버튼을 완료 버튼으로 변환 : 편집 버튼을 감추고 완료 버튼을 보이게하기
            ...
            updateForm.addEventListener("submit", (evt)=>{
                ...
                if (newName === originName && newMemo === originMemo) {
                    // 갮이 변경되지 않았다면 모든 엘리먼트(컨트롤, 컴포넌트)들을 원래도록 복귀시켜놓고 처리를 하지 않음
