...
public class ArticleService {
    ...
    public void saveArticles(NewsResponse newsResponse, String category) {
        try {
            for (ArticleDTO article : newsResponse.getArticles()) {
                ...
                // 이미 기존에 입력되어 있는 source가 있다면 DV에서 찾아서 인스턴스를 만들고
                Optional<Source> scrOpt = sourceRepository.findByName(article.getSource().getName());
                // 없으면 새로 생성(srcOpt 안에 인스턴스의 값이 null임)
                Source src = scrOpt.orElseGet(() -> {
    ...
    public List<SourceByArticleDTO> getArticleCountBySource() {
        // JPA : 자카르타 Persistance
        // JPQL : JPA 전용 Queru Language
