...
public class AdminController{
    ...
    public String categories(Model model) {
        //데이터베이스로부터 카테고리 정보를 가져와서 admin의 카테고리 페이지에 전달한다
    ...
    // categoryName으로부터 전달받은 데이터를 데이터베이스에 저장하라는 request
    ...
    public String inputCategory(@RequestParam("category_name")String categoryName, Model model){
        if (categoryName != null && !categoryName.trim().isEmpty()) {
            ...
            // 카테고리 entity 인스턴스를 생성한다
            Category category = new Category();
            category.setName(categoryName); // name field를 categoryName으로 설정
            ...
            if (msg != null && msg.startsWith("ERROR")) {
                ...
                // templates 폴더 안에 있는 category.html을 랜더링해라
                // Server Side Rendering(SSR)
                return "category";
            }
        }

        //request를 다시 만들어서 해당 request를 다시 요청
